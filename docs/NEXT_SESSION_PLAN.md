...
2. 无外部依赖候选：**以 docs/coverage-matrix.md「可立即实现清单（I 编号）」为准**（每 rc 轮刷新；
   按 owner 指定优先级：**消费者相关（I5 调用概览、I6 MCP 面 JWT 回退）与接口相关（I2 F19 前端页、
   I3/I4 同步与探测端点）先行**；已交付的划除并在表内回链 issue）。
