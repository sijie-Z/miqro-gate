...
- [~] U2 管理面三组逐批切换 —— **组织组并入 develop（PR #134）**；**平台组 8 页完成（PR #135 @ goal/ui-u2-platform-console，CI 中）**；**运营组进行中**：A 批 5 页完成 @ goal/ui-u2-ops-console（导出/用量删除/审计/全局配置/API 消费者；vitest 144/144、Playwright 40/40）；**剩余 B 批 6 页**：技能库管理 / 智能体 / 服务管理 / Webhook 端点 / 告警规则 / MCP 服务（含访问控制弹窗；MCP 30k 行 legacy 最大）
- [ ] U2 管理面三组逐批切换
