...
- 客户端断开时立即取消上游请求。
- **首字节前最多重试一次**：仅当失败是连接阶段错误（`WebClientRequestException` 且非任何超时）且尚未收到首个响应字节时才重试；真实凭证只在第一次尝试前解析一次，重试复用同一凭证。重试次数（`retry_count`）随请求生命周期记录持久化。**该窗口以首字节为界，不以「请求体是否已送达上游」为界**：请求体按 chunked 直通，上游完全可能读完整个请求体之后才断开连接，此时上述判定仍会重发——非幂等请求存在被上游重复执行/重复计费的窗口，而网关侧只记一条生命周期记录（`retry_count=1`），不会因此记两条用量。把门禁收紧到「请求体已下发即不重试」属于产品决策变更，需新增 ADR（#1387）。
- 流式响应一旦开始（首字节已出），禁止重试。
- 禁止跨供应商或跨真实凭证故障切换。
