...
| `MIQROKEY_PLATFORM_OIDC_HTTP_TIMEOUT` | `PT10S` | **每次**出站调用（token 交换、userinfo）的整操作墙钟预算（#1294）：连接 + 请求 + 响应头 + 响应体合计，超时抛 `OAuthFlowException`（AUTH_ERROR / USERINFO_INVALID）走登录页重定向。这是**总时长**上界，不是读空闲——传输层自带的 10 s 只在「两次读之间」计时，对端持续滴字节即可让回调请求永远不返回并占用 Tomcat 工作线程。对端确实慢但合法的 IdP 可调大。**注意本项之外还有第二个计时器**：传输层 `connectTimeout` 固定 10 s（`PlatformOidcAuthService` 的进程级静态 `HttpClient`，不随本项联动）；本项调到 10 s 以上时，**连接建立**仍会在 10 s 处失败，且形状是 500 `INTERNAL_ERROR`（`HttpConnectTimeoutException` 属传输失败），不是登录页重定向 |
...
| `MIQROKEY_UPSTREAM_RESPONSE_TIMEOUT` | `PT10M` | 整体硬截止（**自请求体到手起计时**，**含排队等待**与缓存读/凭据解密/SSRF DNS 等前置跳，不重置）；流式空闲另算 |
...
| `MIQROKEY_GATEWAY_CONTENT_FILTER_ENABLED` | `false` | 内容过滤 shadow v1 开关（#740，ADR-0027，`miqrokey.gateway.content-filter.enabled`）：**默认关**——关闭时不评估、不注册任何指标、不记日志，热路径行为与引入前逐字节一致。打开后网关**进程内**用本地词表扫描**已缓冲的**请求/输出 body，只观察：每次评估自增 `miqrokey_gateway_content_filter_evaluations_total{direction}`（每次评估都加，「无命中」因此与「没在跑」可区分），类别命中自增 `miqrokey_gateway_content_filter_hits_total{direction,category}`（同类别每次评估最多记 1 次），并记录 `miqrokey_gateway_content_filter_match_seconds{direction}`（匹配耗时；overflow 跳过的评估不记）。命中时一行无正文日志 `content filter shadow: direction=… category=… hits=…`。**零阻断、零改写**：不拒绝/不改写/不截断/不注入任何请求或响应；正文只在网关进程内读取，不落盘、不进留痕、不出网关（ADR-0027 §2 D1–D5）；匹配异常就地吞掉（无正文 debug 行），不影响请求 |
| `MIQROKEY_GATEWAY_CONTENT_FILTER_VOCABULARY` | 空 | 本地词表（#740，`miqrokey.gateway.content-filter.vocabulary`）：`类别=词1\|词2;类别2=词3`——类别 = metric tag 有界自由串；词 = **大小写不敏感子串**；重复类别以最后一次定义为准。匹配只看每个 body 前 8KiB 解码文本（有界热路径开销，超出部分不参与匹配）；输出侧被缓冲截断（overflow）的整段跳过匹配但仍计评估。畸形条目跳过并在启动时记一行不含条目内容的告警；开启但空词表亦告警。**词与正文绝不进日志或指标** |
