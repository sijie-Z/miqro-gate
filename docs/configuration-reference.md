...
| `MIQROKEY_QUOTA_ENFORCEMENT_INTERVAL_MS` | `60000` | 配额阻断评估周期（`miqrokey.quota.enforcement-interval-ms`，#684 块①）：按 REJECT 规则重算越线作用域并同步 `quota_enforcement` 投影，单租户评估失败只跳过该租户、不中断整轮；容量注记：与其他 `@Scheduled` 任务共用控制面默认单线程调度池（未设 `spring.task.scheduling.pool.size`），租户/规则量增长时需关注任务排队 |
