...
**写入方（`V55` 建表时缺、#629 补齐）**：网关用量写入器 `PostgresUsageEventWriter` 与 `usage_event` 同批同事务写入；行 `id` = 该笔 usage 事件的 `id`（`ON CONFLICT (id) DO NOTHING`，重放不重复，两表可按 `id` join）。只对使用了外部选择器的裁定写行：`RESOLVED_HEADER` → `source='header'`、`value=` 客户端声明的 project id；`RESOLVED_SUFFIX` → `source='suffix'`、`value=` Key 中呈现的 tag。`SOLE_BINDING`/`POLICY_ROUTED` 未使用任何外部线索（`source` 值域无诚实取值），其解释即 `usage_event.resolution_status`，故不写行。`scope`/`observed_at` 保持默认（网关观察不到客户端侧作用域）。**读取方（查询 API）尚未交付**。
