...
| F20 | 用量差异「追加 adjustment」机制（不覆盖原始事实） | operations-runbook §7 | 清晰（追加语义明确） | **部分交付（2026-09-17，#709/PR 见下）**：V62 表结构 + 追加/查询接口（`POST/GET /api/v1/admin/usage-adjustments`，仅 SYSTEM_ADMIN）+ 反向行纠错 + 幂等键 + 按 `usage_event_id` 串行化的净额非负校验。**余额**：明细净额列、审计/导出调整标记、对账联动 | F19 差异产生后闭环需要 **验收口径**：验收口径：F19 差异确认后写 usage_adjustments(不覆盖原始事实),明细查询带净额列,审计/导出含调整标记。| 架子已建（V62，表结构一次到位并预留 COST 金额维度）；release-checklist 的 adjustment schema 门禁已勾。四层语义（observed / adjustments / AdjustedUsage / 配额隔离）见 database-schema §6 |
