...
>
> 口径图例：`PLANNED`=已立项可做 | `SCAFFOLD`=清晰度不足，先留架子 | `BLOCKED`=外部阻塞（等 leader/平台/凭证） | `ADR`=需决策反转/新 ADR 方可做 | `DECLINED`=刻意不做（红线，防误入） | `DEFERRED`=明确远期（无立即价值） | `TBD`=待核对代码/出处
>
...
| F02 | 缓存键升级为「提取最后一条 user 消息」（对齐腾讯/阿里） | ADR-0009 后果；mapping 启示 1 | 清晰（两家一致做法） | PLANNED | 需回归缓存契约测试（字节一致仍须成立） | 现为全请求 hash；升级只动键构造，键含 tenant+Key 作用域隔离不变 |
| F03 | 模型审批 Webhook 通知（文档「预留」） | progress 审批流边界 | 清晰（复用 alert/webhook 机制） | PLANNED | 无 | 审批事件 → 通知申请人/管理员；不做阻断 |
| F04 | 用户自助配额可见性（个人看自己的配额规则水位） | progress 配额规则边界 | 清晰 | PLANNED | 无 | /me 配额视图（只读展示本人相关规则+水位） |
| F05 | 管理门户 IP 白名单 | security §6；middleware 安全边界 | 清晰 | TBD | 配置项不存在，需先核对代码 | 按 security 规格「支持配置 IP 白名单」落实；推理 API 来源 IP 限制为远期 F40 |
| F06 | 过期导出/删除请求定时清理（GC） | progress G4.4 边界 | 清晰 | PLANNED | 无 | 定时清理 EXPIRED export_tasks 与过期 usage_deletions |
...
| ID | 功能 | 出处 | 清晰度 | 状态 | 前置/依赖 | 架子与要点 |
|---|---|---|---|---|---|---|
| F11 | MCP 路由规则（default 兜底不可改删 + 自定义高优先级；Path/Host/Method/Header 匹配；冲突实时校验；不读正文） | study A4（raw 10） | 清晰 | PLANNED | F01 代理接线后才有调用面（配置面可先行） | 规则表 + 匹配纯函数 + 冲突校验；default 路由强制存在 |
| F12 | Tools 重试门禁（非幂等显式确认；首字节前才重试；默认关闭） | study A6（raw 12） | 清晰 | PLANNED | F01 | 护栏语义与网关「首字节前一次重试」同源，工具层加显式开关 |
| F13 | Tools 熔断（三态；最小请求数防误判；慢调用阈值 < 后端超时校验；熔断期跳过重试；429 可触发） | study A7（raw 13） | 清晰 | PLANNED | F01 | 状态机纯函数 + 默认关闭；阈值校验规则明确 |
...
| ID | 功能 | 出处 | 清晰度 | 状态 | 前置/依赖 | 架子与要点 |
|---|---|---|---|---|---|---|
| F24 | 默认配额模板（全局模板；创建时快照复制；改模板不惊动存量；关闭不删已分配；手动规则覆盖默认） | study A10（raw 22）；roadmap 配额 | 清晰（语义完整） | PLANNED | 无 | 快照复制语义与 budget 追加式同族；可借鉴到预算模板化 |
