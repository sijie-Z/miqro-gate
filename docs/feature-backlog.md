...
>
> **2026-09-10 配套**：大厂能力全景见 [coverage-matrix.md](coverage-matrix.md)（29 篇逐能力四态定级 +
> "可立即实现/需裁决"清单，每 rc 轮刷新）——本表管"已立项登记"，coverage-matrix 管"能力维度底账"。
>
...
| F07 | 告警类型补齐（usage 队列饱和/解析失败/供应商错误/Plan 同步/磁盘等 → alert_rules 类型） | progress G4.5 风险；release-checklist §6.1 | 部分（多数类型定义清晰，数据源需接线） | SCAFFOLD（**部分接线，2026-09-16 #245**） |各类型数据源接线 **验收口径**：验收口径：配置各类型规则后,对应数据源事件能产出 alert_events 且去重/退避/投递同既有链路；至少 usage 队列饱和与解析失败两类先接（队列全局性需先定承载租户口径）。| **2026-09-16 回填**：① usage 队列饱和已接线（#245）——V60 `gateway_queue_signal` 事实表 + 网关丢弃上报 + `USAGE_QUEUE_SATURATION` 评估 + 类型注册 8 处（核心 4：服务校验列表/前端联合类型/下拉选项/评估分支；配套 4：抛错文案、迁移 CHECK、i18n 词典、API 契约）；承载租户口径定为「默认 seed 租户 + 评估按 `tenant_id` 过滤」，非 seed 租户同名规则在正阈值下恒不触发、读不到他租户数字（有测试，含 `threshold = 0` 的退化行为固定用例）。② 「解析失败」与「供应商错误」经核对**本就已接线**，无需新数据源：二者均由 `usage_event` 派生（`USAGE_MISSING_RATE`/`UPSTREAM_ERROR_RATE`），本轮同时补上租户过滤（此前 SQL 未带 `tenant_id`，属多租户前的漂移）。③ Plan 同步 / 磁盘等仍无数据源，保持 SCAFFOLD。**2026-09-20 回填**：#245 平台事件模型定稿并合入（`docs/platform-event-model.md`），owner 拍板 Q1–Q4 全部按推荐 A（单承载点维持 / 一信号一类型 / 宕机 >1h 盲区接受并文档化 / 现状+文档修正）；后续新平台信号按该文档 §4 的「8 处注册面」清单接入。| 剩余类型逐个接线，验收口径同上 |
| F08 | 官方价格 24h 自动同步（source=OFFICIAL 自动化） | progress G7.2 风险 | 部分（依赖供应商官方价格源） | **DONE（2026-09-17，#708）** |供应商价格源确认 **验收口径**：验收口径：官方价源确认后,24h 定时拉取 → price_snapshot source=OFFICIAL 增量写入(与 MANUAL 并存,同模型同 type 冲突策略=保留 MANUAL 并提示)；失败留日志不静默。| 交付：价源沿用 #585 的 OpenRouter 适配器（官方模型索引）；`PriceSyncScheduler` 按 `MIQROKEY_PRICE_SYNC_AUTO_CYCLE_MS`（默认 24h，fixedDelay）复用同一同步管道增量写 `source=OFFICIAL`，开关 `MIQROKEY_PRICE_SYNC_AUTO_ENABLED` 默认关（ADR-0020 保守默认）；冲突策略=同键最新为 `MANUAL` 且价格不同则保留人工价、计入 `conflicts`（报告 + 审计摘要；人工触发端点保留其"覆盖"契约）；失败零写入且不静默（`PRICE_SYNC_FAILED` 审计 + 调度器 ERROR 日志 + `miqrokey_control_price_sync_auto_total{result}` 指标，本轮异常吞掉不影响下一轮） |
...
| ID | 功能 | 出处 | 清晰度 | 状态 | 前置/依赖 | 架子与要点 |
|---|---|---|---|---|---|---|
| F19 | 官方账单导入 + 自动差异报告（四级匹配：request ID → 指纹+模型+时间 → Token/费用 → 时间窗） | usage-accounting §11；roadmap 后续 | 部分（匹配规则文档清晰；账单格式依赖真实样本） | **IN_PROGRESS（2026-09-10：#301 引擎 + #334 端点层/V42 已交付、IT 2/2**；残留：供应商私有解析器与指纹级匹配 WAITING_FOR_SAMPLE、前端页/导出 follow-up） |真实账单样本（任一供应商） **验收口径**：验收口径：任一家真实账单样本到位后,导入解析→四级匹配(重名覆盖由明细级去重)→差异报告页可看可导出；无样本阶段保持 SCAFFOLD。| api-contract §6 规格已超前：异步任务 + MATCHED/PARTIAL/UNMATCHED_LOCAL/UNMATCHED_PROVIDER；契约稿 docs/bill-reconciliation-contract.md（#293）已定 canonical v0 与端点契约；供应商解析器待样本 |
| F20 | 用量差异「追加 adjustment」机制（不覆盖原始事实） | operations-runbook §7 | 清晰（追加语义明确） | **部分交付（2026-09-17，#709/PR 见下）**：V63 表结构 + 追加/查询接口（`POST/GET /api/v1/admin/usage-adjustments`，仅 SYSTEM_ADMIN）+ 反向行纠错 + 幂等键 + 按 `usage_event_id` 串行化的净额非负校验。**余额**：明细净额列、审计/导出调整标记、对账联动 | F19 差异产生后闭环需要 **验收口径**：验收口径：F19 差异确认后写 usage_adjustments(不覆盖原始事实),明细查询带净额列,审计/导出含调整标记。| 架子已建（V63，表结构一次到位并预留 COST 金额维度）；release-checklist 的 adjustment schema 门禁已勾。四层语义（observed / adjustments / AdjustedUsage / 配额隔离）见 database-schema §6 |
| F21 | usage_event 延后列批量落地（**已收敛为价格快照**；其余见拆分项 F21-B/C/D） | database-schema §6 | 清晰（2026-09-18 评审收敛） | **部分交付（2026-09-18，F21-A）**：V64 价格基座列 + 确定性 tie-break + 历史回填端点（按 `occurred_at`、`UNAVAILABLE` 不伪造）。**余额**：成本读取改走基座 |与 F19/成本重算语义绑定 **验收口径（2026-09-18 修订）**：见 issue #710 评论——事件生成时冻结价格、价格发布不改历史、回填按 occurred_at、无法恢复须显式标记、**所有**成本入口最终统一读快照（不再是"至少一处"）| **拆分**：F21-A 价格快照（本轮）/ F21-B 归属快照（先定「事件时刻归属」语义）/ F21-C 上游 usage 原文（独立合规 ADR）/ F21-D 派生元数据（error_category 等，口径稳定再定）。**不加** `price_catalog_version`（无版本化目录实体，凭空加是假装存在）与成本列（与 #709 调整层冲突，成本应为派生量） |
...
| F51 | 配额硬阻断（超限拒绝） | middleware 配额 | **DONE（#684，2026-09-16）** | owner 拍板「软着陆：拒绝请求」→ **ADR-0020**（采纳 ADR-0019 选项 B：规则级 opt-in `action=REJECT`，429 + `quota_exceeded`，Key 不失效）；落地=控制面判定集 `quota_enforcement`（V59）+ 快照 + 网关准入门；不做限流、不做数据面计数（ADR-0020 D6/§3-D） |
...
| ID | 功能 | 出处 | 清晰度 | 状态 | 前置/依赖 | 架子与要点 |
|---|---|---|---|---|---|---|
| F60 | 管理开放 API：机器对机器管理凭据 + OpenAPI 对外发布 + curl/Python 示例（+可选频控/作用域） | leader 2026-09-07；腾讯 AI 网关既有形态 | **DONE（2026-09-09：#268 批 2 v2 委托建钥案 1；此前 #251 写面 v1 A+C 与契约/示例随 rc.4 轮）** | ADR-0016 Accepted（A+C）+ 2026-09-09 增补 | 交付：读面（#204）+ 写面 v1（告警规则/Webhook 机器全生命周期；导出创建委托=发行管理员）+ 批 2 v2（Virtual Key 委托创建：机器代 target 建钥、钥归属目标、成员边界按目标、审计 actor+targetUserId，ADR-0016 增补）；curl/Python 示例与最小权限建议。批 3 scope（能力组）2026-09-09 已交付（#298）；到期提醒 UI/事件为 follow-up | 现状=/api/v1 全量 REST + OpenAPI 3.1 + CI breaking；先例=api_consumers/JWT(ADR-0010/11)。批1 V32 admin_api_key（SHA-256 digest + prefix，Bearer 鉴权，审计；读子集先行）→ 批2 契约/示例（写面等 ADR-0016）→ 批3 治理（作用域/频控,可选） |
