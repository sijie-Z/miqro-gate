...
- **登录页（2026-09-07 增补：设计师权威稿为现行母版，覆盖上方 v3 登录段）**：左右分屏——
  ...
  纵向单列能力卡、底部信任条）；右白色认证面板（品牌行、欢迎回来标题、账号/邮箱与密码 44px
  控件、紫→蓝渐变主按钮、or 分隔、申请账号卡、数据保护卡、隐私条款页脚）。实现以
  `NextLoginView.vue` scoped style 为准；登录页特许渐变/辉光（aesthetic audit 豁免区），
  与控制台其余页面「零渐变」纪律分开。
  母版时间线：#194/#196 设计师稿对齐 → #212 最终打磨（对照评审 7.5→8.5）→ #214 全页中文化。
...
- 主内容最大宽 1200px（`--ui-content-max`，"固定"模式）；"流式"模式解除该上限，页面带铺满视口，表单正文限制在 760px。
...
以表格为主。名称和掩码在第一列，固定绑定拆成项目、产品、用途、模型列。状态使用小型 label。行操作一律是可感知的链接型操作（蓝色文字，hover 加深并出现下划线；危险操作为红色链接）：高频动作直接铺开，低频动作收进「更多 ⌄」下拉。禁止在操作列使用无边框 ghost 按钮或纯图标触发器（#651）。

创建 Key 使用单页表单而非五步“炫技向导”；字段有依赖时逐步展开。成功后的 Secret 使用专用一次性 Modal：白底、深色等宽 Key 区域、复制按钮、明确不可恢复警告，无彩纸动画。

### 页面使用指引（PageGuide，#656）

每个管理页页头下方可有一条「使用指引」卡（`UiPageGuide`，内容在 `src/content/pageGuides.ts`）：3–4 步链路，每步 = 动宾标题 + 一句话 + 「前往『X』」跨页链接（可选接入文档外链），承担腾讯云控制台「产品指南」的角色——首次进入就知道这个页面在一件事的哪一步。它是说明书不是流程：不做多步向导、不替代表单；可「收起」为一行细条、「不再显示」，两种状态按页记忆（localStorage）。涉及"改完是否要同步"的步骤必须写明「保存后数秒内生效，无需同步」（本产品快照自动刷新）。新增文案全部进 EN 词典。

### Provider / Plan

供应商 logo 只作为 20px 辅助识别，不做彩色大卡片墙。产品列表必须直接显示协议、Plan 形态、验证状态、最近同步、Base URL host。「实现状态」徽章 hover/聚焦出示状态解释（对应 docs/provider-catalog.md §2 的状态阶梯），行操作提供「接入文档」外链到该文档对应供应商章节。团队 Plan 详情根据共享池/席位/成员 Key 呈现真实结构。

### 接入指引对话框（CcSwitchImport，#839）

「我的密钥」的两个对话框（创建成功弹窗 / 行级「接入指引」）共用 `CcSwitchImport` 组件，结构固定：
- 上半区=一键导入：目标应用段选（Claude Code / Codex，默认按密钥用途推断）+ 导入按钮 + 结果反馈卡（唤起检测三态：发送中 / 已唤起含三步指引 / 未检测到含兜底路径）+ 「复制导入链接」；发送后有约 4s 冷却，防连点造成重复导入。深链为单向协议（CC Switch 不回执），反馈文案不得承诺导入成功。
- 下半区=手动配置：客户端段选 × 终端/平台段选；每个配置文件独立成块（`文件路径标题 + 复制按钮 + 片段`），Codex 区分 auth.json / 环境变量两种凭据位置；模型以 chips 呈现（默认取密钥已授权模型），选择结果联动片段与导入深链的 `model` 参数。
...
- 自动检查不出现 `linear-gradient`、`radial-gradient`、大于 8px 的常规容器 radius，以及未批准的紫色。紫色检查（`frontend/src/__tests__/aesthetic.spec.ts`）**按色相判断**并覆盖 `src/**/*.{css,vue,ts}` 的普通写法（hex 3/4/6/8 位、`rgb()`/`hsl()`/`oklch()`、紫色系颜色名）——它曾只扫 CSS 且只比对固定色值黑名单，让一个 `.ts` 里的紫色色板溜过（2026-09-20，见 `chart-palette.ts` 注释）；供应商 chip 与注释里提到的色值不触发；**已知盲区**：3/4 位**纯数字**十六进制只在「颜色位置」按颜色读（字符串字面量里、CSS 取值位 `color: #639`/`--c: #639`、或 `.css` 文件里）——用这种方式把 `#758` 这类 issue 号与 `#639`（rebeccapurple 缩写）区分开；裸写在 TS/Vue 语句里的纯数字缩写按票号跳过。另有：运行时拼出来的颜色、`oklab()` 与 `color-mix()`。
