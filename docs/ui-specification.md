...
1. 选择项目。
2. 选择明确命名的供应商产品/Plan。
3. 选择用途和允许模型。用途是声明性标签、不限制客户端（#596 起表单内显性说明）；实际可调用范围由授权产品与允许模型决定。
4. 确认固定绑定并创建。

创建成功弹窗只显示一次 Base URL 和 Virtual Key，分别提供复制按钮，并要求用户勾选“我已保存”。关闭后不可恢复明文。弹窗内提供「导入到 CC Switch」（深链预填其确认框，目标应用可选 Claude Code / Codex）与手动配置片段；导入为单向深链、无回执，门户以唤起检测 + 分步指引呈现反馈（CcSwitchImport 组件，#839）。
...
展示账号信息、修改密码、当前会话和退出其他会话。首版无 MFA。（#597 交付：身份头 + 用量速览 + 账号/安全/当前会话分区；「退出其他会话」= `POST /api/v1/auth/logout-others`。）
