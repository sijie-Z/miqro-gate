...
**价格从哪来（#710 / F21-A 起）**：每个用量事件携带**它自己采用的单价**（`usage_event.price_*`，V64）。口径是「**行内冻结值；缺失则取该行 `occurred_at` 时刻的价目**」。

> 这一版之前，实现实际用的是**查询时刻的最新价目**——与本节的表述不符，后果是改一次价目，**历史报表金额跟着变**。本刀让代码对上了它自己的规格。

- **未定价 ≠ 免费**：查不到价格时成本记 0，但行上保留 `price_status = UNAVAILABLE` 且价格列保持 NULL。两者含义不同，不得混同
- **缓存节省（`savedByGatewayCache`）**：命中行没有对应的用量事件，因此按**该组命中中最晚一次的时刻**取价目。**这是近似**——一个 cache_key 的命中若横跨改价，整组会按较晚的价计价
