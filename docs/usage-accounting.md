...
**界面上也要说得出这句话**（#801）：当 `pricingStatus != COMPLETE` 时，展示成本数字处带一个「未定价」标记，气泡给出缺口规模（未计价事件数 / 其中完全无法定价的数）。API 从 #766 起就带着这个事实，但控制台此前**一处都没渲染**——界面上那个金额看上去就是总额，而本节刚刚说过它不是。`COMPLETE` 时不加任何标记（没有缺口就不该有噪音）。缓存节省侧同理：`unpriced.unpricedHitEvents > 0` 时节省额标为**下界**（#790）。
