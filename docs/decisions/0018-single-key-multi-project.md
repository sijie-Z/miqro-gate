...
**修订（#1117，PH46 bug hunt）**：D7 的项目粒度绑定禁用**不因密钥自身状态而窄化**——`DISABLED`（#582 自助停用）的 Key 同样要被置灰其绑定，否则属主一次自助 `enable` 即可恢复路由，"成员移除"静默失效。相应地，`REVOKED` 兜底仍只作用于 `ACTIVE`/`ROTATING` 密钥：被停用的 Key 在失去全部绑定后保持 `DISABLED`（仍可由本人启用，但不再有任何路由），不被级联吊销。
...
| 枚举探测 | 无效密钥/畸形密钥 → 全部统一 404，响应路径一致（沿用既有防枚举约定；模型越权 403 的例外维持不变——请求方自知所询模型，不构成额外信息泄露）。**修订（#633，2026-09-16）**：标签不匹配不再一律 404——单绑定 Key 上标签是装饰性的，多绑定 Key 无上下文时 `400 CONTEXT_REQUIRED`（见 `docs/context-attribution-implementation-spec.md` v1.1 §4） |
