...
### 2026-09-22：选项 C 转为「有条件采纳」（**当前状态以本节为准**）

- **采纳：选项 C（Key 级 opt-in 整流重试）进入首期实现范围**，但**只允许 thinking / signature 整流**；
  `budget_tokens` 划归**二期**；**硬条件见 §0 的八条**（白名单错误模式、仅首字节前、最多 1 次重试、
  不改 messages 正文、不记正文、计费语义明确、无副作用证明的供应商不整流等）。
- **未采纳 / 二期待决**：**D**（独立重试预算）、**E**（预算类整流），以及 **C 中被划到二期的
  `budget_tokens` 部分**。它们各自仍需 §2 的例外边界与红线文本修订，按 §3 的触发条件再议。
- **§4 中与 C 相关的设计**（整流语义、字节策略、整流后不写缓存、响应头声明等）**自本日起进入实现范围**，
  但**必须同时满足 §0 的每一条**；与 D / E 相关的部分仍为提案。

> **早期表述已失效**：本节此前写的是「C（仍为 Proposed 状态、不得据此实现）」。
> 该表述自 2026-09-22 起**被上一段取代** —— 记在这里是为了让读到旧版本的人知道它已作废
> （review #1365 指出：同一份 ADR 曾同时告诉实现者「C 可实现」与「C 不得实现」，属必须清的内部矛盾）。
