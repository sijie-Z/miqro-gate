...
3. **只允许「加标记」**，不得顺手变成泛化的 request optimizer：
   ...
   - 注入点与数量有**明确上限**；
   - 有 metrics；
   - **有审计记录，且「审计」在此有确定含义**（review #1365 的 P2 —— 原文只说「有审计事件」，两种读法都通）：
     - **A（本 ADR 采用）**：**Key 的开关 / 配置变更**写入 `admin_audit_events`；
     - B（**不采用**）：「每次发生注入都形成一条审计事件」。
     **逐请求**的注入事实走 **metrics + 生命周期字段 + 响应头**（本文件 Q3 的设计）；
     网关当前**没有逐请求写 `admin_audit_events` 的通道**，本条**不**引入它。
   - **审计 / 指标都不记录正文**；
...
| Q6 | 折中：默认关、只添加、≤4、已有标记保留、审计记注入发生、开启后不再字节等同 | **已采纳（2026-09-22，边界以 §0 为准）**，并按 §2 的例外边界收紧（「审计记注入发生」按 Q3 修正为指标 + 生命周期字段） | §2、§4 |
...
## 3. 选项与代价（**历史候选方案**；当前约束以 §0 为准）

> 本节记录提出时的选项比较与推荐，**用于还原决策过程**，不是当前的待决清单。
> 2026-09-22 owner 已明确选择：**Key 级、默认关闭、只加标记、不删除已有 `cache_control`、不重排、
> 不修改 messages、有上限、有 metrics、有审计（含义见 §0）**。凡本节与之冲突的，**一律以 §0 为准**。
