...
- **缓存键已经含「语义」成分，且完全在网关内完成**：`CacheKeyFactory.compute()`（`:68-76`）对 chat 形态请求使用 `semanticScope(body)`（`:102-142`）——取 system 消息 + **最后一条 user 消息**拼接后参与 SHA-256；无 user 消息时回落到全 body 归一化哈希。键还含 tenant/project/keyId/product/model/purpose/stream/family 维度。这正是 `docs/ai-gateway-comparison.md:92` 记录的「我们语义键=末条 user 消息哈希」。
