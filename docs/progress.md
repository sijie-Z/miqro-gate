...
- **反向发现**：全仓 `docs/*.md` 检索 `唯一归因|唯一身份|一笔一|每笔绑定|可归因`，**除本条目自身与 ADR §1.3 的同一句之外零命中**——#704 当作一等约束的不变式，此前只存在于代码与 DDL，本 ADR 是它第一次被写成文字。
...
- **未决项**：7 条待 owner 拍板 + 4 条需 owner 补充的事实 + 4 条本 ADR 明确不回答（留待各自议题）；其中「是否存在同产品多凭证供给」是**能力是否可用的前提**——若生产不存在「同一产品、不同 subscription、两把以上 ACTIVE 凭证」，则候选集为空，应先解决凭证供给而非实现回退。§2-Q7 另记一条实操警告：**同账号内多把凭证回退不缓解账号级 429，反而可能加速封禁**。
- **验证**：本线为纯文档改动，无代码/测试可跑。已执行的核查：编号核查（上述）、`README` 索引行与文件名一致性、以及对本 ADR **实际引用**的坐标逐条 grep 复核。**实际引用集合（脚本从 ADR 正文抽取，56 条，含简写续引）**：`V1__core_tables.sql:207/210/288-291/309/359/373-374`、`V6__usage_events.sql:24/47`、`V8__request_usage_records.sql:39/55/69`、`V9__quota_snapshots.sql:16`、`V57__unattributed_policy.sql:8`、`ProxyController.java:95-97/445/454/471/501/526-530/559/571/632/634-635/649-661/663`、`RouteSnapshot.java:251`、`HeaderFilters.java:77`、`AdminCredentialService.java:151`、`SseReplayEngine.java:21-22`、`PostgresUsageEventWriter.java:181`、`PostgresUsageEventWriterTest.java:137-147`、`api-contract.md:1003`、`architecture.md:155-157/159/160/161`、`CLAUDE.md:35/36`、`product-requirements.md:27/33`、`provider-adapter-contract.md:125/126/127/128`、`ai-gateway-comparison.md:33/93/94/123`、`tencent-ai-gateway-mapping.md:22`、`feature-backlog.md:109/113`、`operations-runbook.md:99`；另有不带行号的引用（`AuthContext.java`、`V64__usage_event_price_snapshot.sql`、`V66__usage_event_base_cost.sql` 及关联 ADR/文档链接）。首轮修复摘要曾把**未被引用**的坐标（`V4`、`V53`、`V57:20`、`PostgresUsageEventWriter:116`、`ProxyController:648-663` 等）当作已复核项列出，本条已改为脚本抽取的真实集合。**首轮独立对抗评审判定 CHANGES_REQUIRED**：6 处事实/坐标错误（4 处行号错位、1 处把 javadoc 误当入站剥离清单、1 处把「响应阶段失败」误述为「今日不产生用量事件」）已逐条修复并复跑 grep。**本条目早期版本曾声称「未使用任何未命中的行号」，该声称不成立，已删除。** 第二轮复核再修 1 处：`api-contract.md:1002` → **`:1003`**（develop 侧 #776 在 §5.6b 上方增删行导致该段整体位移，已按新坐标 grep 复核）。
