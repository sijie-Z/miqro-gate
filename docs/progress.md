...
- **编号**：实际用 **0022**，未顺延。原因：`origin/develop` 现存 `0001–0020` + `README.md`，**0021 已由并行线预分配**
  （模型路由/故障切换线），故取 0022。
...
- **未完成（下一步）**：本条目落盘时 ADR 仅本地提交，尚未 push、未开 PR、未在 #718 发裁决选项评论；
  对抗评审须在 push 前完成。下一步 = 跑独立对抗评审 → 修/反驳至共识 → push 分支并按 PR 模板开 PR（引用 #718，
  **不写 `Closes`**，本线不关单）→ 在 #718 发评论给 owner 拍板选项。
