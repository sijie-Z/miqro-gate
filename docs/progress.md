...
- **未完成（下一步）**：ADR、索引行与本条目均已本地提交（HEAD 67652354），尚未 push、未开 PR、未在 #718 发评论。
  交付前置的独立复核已完成两轮：第 1 轮 5 项 blocker（不存在的枚举值、厂商页必填/可选倒置、无具名来源的加粗断言、
  本条目早期用词、Azure 运行时 URL 归属）全部闭合；第 2 轮新增 1 项（厂商旋钮归属与 `disabled` 释义）已按唯一来源
  更正，待复核确认。下一步 = 复核结论落盘 → push 分支并按 PR 模板开 PR（引用 #718，**不写 `Closes`**，本线不关单）
  → 在 #718 发评论给 owner 拍板选项。
