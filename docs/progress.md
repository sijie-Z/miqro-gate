...
- **形态**：猎线（审计+修复），不是 Goal。真起**两个控制面实例**（不同端口、同一个 PostgreSQL 15524）逐条核 §1 五类多副本风险：定时任务重复执行 / 分布式锁 / 投递重复 / 本地缓存视图不一致 / 启动竞态。确认 2 个缺陷（#1383、#1394），其余逐条否证（报告 `_orchestrate/reports/PH87_report.md` §5，内部对抗评审 `_orchestrate/reports/PH87_review.md`）。
...
- **崩溃语义（真杀实例验证）**：崩溃者只留下「已抢占、未发送完」的那一行（`attempt = dueAttempt`）；租约到期后下一个扫描重新抢占**同一行**、发出 `dueAttempt + 1`——**与崩溃那一次正在发的 attempt 号相同**（不是新开一次），所以崩溃代价只是一个退避周期的延迟，**不额外消耗 `MAX_ATTEMPTS` 的次数**，两次 POST 的 body 逐字节相同。原始时间线 `_orchestrate/_logs/ph87-crash/run3.txt`：租约到期 `07:48:09.683829` → 幸存者抢占 `07:48:09.732733`（+49 ms，该行 `next_retry_at` 随即被推到 `07:54:09.732733` = 抢占 +360s，租约指纹）→ 接收端第 2 条 receipt `07:48:09.802Z`（`bytes=208`、sha256 与第 1 条一致）；此刻账本仍只有 `attempt=1` 一行（幸存者那次 POST 的 500 要 120s 后才落库）。`pg_advisory_xact_lock` 全库仅 4 处非调度用途；事务级咨询锁随后端终止自动释放（真杀实例后 `pg_locks` advisory 计数为 0），故调度类任务**不能**用事务级咨询锁覆盖一次出站 POST。
- **真库证据**：`AlertDeliveryConcurrentSweepIntegrationTest`（2 例，真 PostgreSQL，`@Tag("integration")`）——两副本并发扫同一到期行，接收端计数 `received.get() == 2`（首投 + 一次重试），投递表 2 行。**注意跑法**：`control-plane-app/pom.xml:161` 有 `<excludedGroups>integration</excludedGroups>`，默认 `verify` **不会执行**该用例（整组被排除）；必须 `-Pintegration`，否则「跑过了」是错觉。最隐蔽的形态是 `-Dtest=<类名>` 单跑而不加 profile：静默 `Tests run: 0` 且 `BUILD SUCCESS`（本仓库既有教训）。实测默认 `verify` 在该模块跑的是 1068 个单测、0 失败，随后在 Spotless 阶段 `BUILD FAILURE`（`_orchestrate/_logs/ph87-e2e/ph87_module_verify_1383.log:8874` / `:8897`）——**不是**「`Tests run: 0` 却 `BUILD SUCCESS`」。绿色原始输出 `Tests run: 2, Failures: 0, Errors: 0, Skipped: 0` + `BUILD SUCCESS`。双实例真进程侧的绿证据：修复版**对同一次 attempt 2 只有实例 A 打印**（14:42:03.986），实例 B 在同一时刻该事件无输出，接收端 1 条；且 attempt=1 的 `next_retry_at` 被推后到 arm 时刻 +360s（= 端点 300s 超时 + 60s 松弛的租约指纹）。**口径须知（勿误读成「B 永远不投」）**：实例 B 后来在 14:46:04 投了同事件的 **attempt 3**——那是租约到期后**另一次独立抢占**，属正确行为。本修复的断言是「同一个 attempt 号只被一个副本发一次」，不是「同一事件只由同一副本发」。
...
- **#1394 对账恢复误伤活着的副本（已立案，修复不在本线 PR）**：`ReconciliationService.recoverInterruptedRuns()` 在**任何一个实例启动时**无条件执行 `UPDATE reconciliation_reports SET status='FAILED', error_message='interrupted by restart' WHERE status IN ('PENDING','RUNNING')`——没有「这行属于哪个实例 / 那个实例是否还活着」的谓词。于是副本 B 的一次普通重启就把副本 A **仍在跑**（实测 `pg_stat_activity` 里 `wait=Lock/relation`、正在 INSERT `reconciliation_rows`）的对账判死。后果不止一条死行：`SELECT_LIVE_REPORT` 的去重谓词是 `status <> 'FAILED'`，判死即去重洞打开 → 同一张账单重传即第二次导入（实测 2 份报告 / 8 行明细 / 4 个不同 `provider_row_ref`、上传身份只有 1 个，审计 `RECONCILIATION_CREATED x3`）。红链原始输出 `_orchestrate/_logs/ph87-e2e/ph87_repro2.out`。**PR 归属**：本线只立案不改代码——`wt-ph90-recon1394` 已持有该修复的在制改动，避免两条线对同一文件开竞争 PR。
- **未立案候选（报告 §5 逐条给否证/未证理由）**：本地缓存视图不一致（否证：A 改 B 立即可读，DB 是唯一事实源）、启动迁移竞态（否证：Flyway 自己的迁移锁，76 行 `flyway_schema_history` 无重复 rank / 无 NULL checksum）、`QuotaSnapshotService.refresh` 双副本重复 append 快照行（读等价，判低价值）、`McpHealthChecker`/`ServiceHealthChecker` 的读-比-写探活竞态（无红证据，不立案）。
- 分支 `fix/ph87-multi-instance-consistency`；issue #1383 / #1394；#1383 的修复 PR **#1404**（https://github.com/sijie-Z/miqro-gate/pull/1404，base `develop`）**已合并**（merge commit `cafc5e1e`，2026-09-22T10:41:09Z；评审后追加的并发用例「保鲜闸门」修正是该 PR 的第 4 个提交 `0cd5fa63`）；本交接点的文字订正另开一个小文档 PR **#1435**（https://github.com/sijie-Z/miqro-gate/pull/1435，base `develop`，不在 #1404 内）。
...
- **行尾纪律（本批踩到两次）**：`CHANGELOG.md` 与 `docs/progress.md` 以 CRLF 存储，本机 autocrlf=true + `* text=auto` 会把整文件归一化成 LF（一条条目显示成 673 行 diff）；本批已给这两个文件加 `-text`（.gitattributes），并一律用「develop 原始字节 + 字节级插入」改写。
...
## 2026-09-19 面向用户的「用量」页：同一屏四处成本展示，让它们口径一致（#877）

### 问题

真机上按「渲染了 `¥`」逐处核对时读到的：**同一屏、同一次查询，四处成本展示两种口径。**

| 位置 | 改动前 | 标记 |
|---|---|---|
| 顶部统计卡 · 上游成本 | `¥0.00` | ❌ |
| 分组表 · 每组「网关观测成本」 | `¥0.0000` | ❌ |
| 分组表 · **合计**行 | `¥0.0000 ¥0.0000` | ✅ `未定价` |
| 最近记录 · 每行成本 | — | ✅ `未定价` |

`¥0.00` 正是普通用户进入用量页第一眼看到的数字，而它**恰好是最没有解释的那个**。
页面**已经**算了 `costCaveat`（`costGapNote(summary.totals)`，合计行在用），只是另外两处没用它。

### 改法

- 统计卡：`summaryStats` 增 `caveat`，按 #849 给首页 `StatCard` 立的同一形状
  （`interface StatCard { …; caveat?: string }`），三张卡里只有成本卡带它
- 分组表：新增 `groupCostCaveat(row)`，走同一个 `@/lib/usage-pricing` 入口。
  每组自带 `pricingStatus` / `unpriced`（API 自 #766 就在发），此前只有行级单元格忽略它们
- 标记位置对齐本页既有约定：**整行一个 chip，放在两个成本数之后**——与合计行完全同形，
  而不是给两个成本列各挂一个

### 一处 TS 细节（值得记）

`UiTooltip.text` 是必填 `string`。模板对 **ref** 的 `v-if` 会做类型收窄，
但对**函数调用的结果**不会——`v-if="groupCostCaveat(row)"` 与 `:text="groupCostCaveat(row)"`
是两次独立调用，vue-tsc 报 `Type 'string | null' is not assignable to type 'string'`。
故该函数返回 `string`，用 `''` 表示"无话可说"，并在注释里写明原因。

### 验证

- **先证明会红**：只回退 `NextUsageView.vue` → 两条标记用例失败
  （`expected false to be true`），"无缺口时不标注"那条照常通过
- lint（`--max-warnings 0`）/ typecheck / **454 tests** / build / **`gen:types` 幂等** —— 全 PASS
- Playwright **59/59**
- 纯前端改动，无后端与契约变化；`gen:types` 幂等、OpenAPI 基线未动
...
## 2026-09-20 前端页面验收：32 条路由全走一遍，唯一缺陷是「允许模型」看不见（#1013）

### 怎么扫的

用内置浏览器 + 本地反代（`demo-live`，Origin 重写）登录线上栈，按 SPA 逐条走完 **32 条受控路由**（总览 / 密钥 / 用量 / 成本 / 缓存收益 / 用量报表 / 用户团队项目 / 授权 / 审批 / 配额 / 导出 / 对账 / 删除 / 消费者 / MCP / 留痕 / 告警 / Webhook / 审计 / 配置 / 智能体 / 技能库 / 帮助 / 资料 / 设置……），每页读四类信号：错误边界文本、表格行数、`NaN` / `Invalid Date` / `1970-01-01` / `[object Object]` 伪值、以及"这一页到底有没有内容"。再叠两轮结构性检查：① **13 个列表页的截断审计**——判据是"`scrollWidth > clientWidth` 且无 title、无 tooltip 祖先、又不在滚动容器里"；② 桌面（1440 / 1920）与移动（375）两档视口的溢出检查。

结果：**32 条路由零白屏、零伪值**；13 个列表页只有**一格**截断且无处可看（见下）；三档视口都没有页面级横向溢出（宽表在自己的卡片里横向滚动）。交互抽查也过：接入指引对话框（Claude Code / Codex 切换、手动配置三客户端、无明文密钥时导入按钮禁用）、界面设置抽屉（菜单主题 深色↔浅色、灰色模式开关——直接读 `<html>` 上 `data-*` 属性的翻转，等于复验了 #828 那类"偏好静默失效"）、复制按钮（环境未聚焦时给出"复制失败，请手动选择复制"的错误 toast，**说明失败路径是有反馈的**）。

**一个测量坑**：应用窗口最小化时 `window.innerWidth === 0`，所有布局尺寸都退化成 32 / 48px 之类，扫描会报出满屏假"截断"。**先确认视口非零，再谈布局结论**——用固定视口（1440×900）后才拿到真数字；DOM 级信号（文本、行数、错误边界）不受影响。

### 唯一缺陷：允许模型被省略号吃掉（#1013）

`/app/keys` 的「允许模型」格是 `nowrap + ellipsis`，实测 `clientWidth 480 / scrollWidth 528`——**第 4 个模型名看不见**；而这一格**没有 title、没有 tooltip**，行内「更多」菜单只有接入/轮换/重命名/停用/吊销，页面上没有第二个入口。同表格的「用途」列早已是 `UiTooltip` 包着的，所以这是**漏了一格**，不是风格取舍；`docs/ui-specification.md` §5 也写着"不能隐藏关键绑定信息"。

修法：按同页惯例包一层 `UiTooltip`（文本=完整清单，空则 `—`），加一条**先证明会红**的回归测试（断言该格存在 `.ui-tooltip__anchor` 祖先；把修复撤掉实测报 `expected null not to be null`，装回即绿）。
...
## 2026-09-20 边打探针边读代码：一次导出让管理面整体 502，以及写面校验缺口族（#1019 / #1021 / #1023）

用户的要求是"像 baiye 那样找 bug"。这条线此前主要在"验收既有功能"，这一轮换成**主动攻击面**：先按四个判据写探针，再对着探针命中的地方读代码。

### 判据（写在探针最前面）

1. **不许出现 5xx**——本仓自己的规矩：泛化兜底会把真实类别藏起来（runbook §15），所以任何 5xx 都是缺陷；
2. **拒绝要说出真实原因**——409 有两个意思，只有 `code` 分得清；"太长"被答成"重复或引用不允许"就是错的；
3. **前端拒绝 ≠ 后端拒绝**——同一份负载，浏览器挡住、curl 写进库，就是"认真用户被挡、绕过的人写脏数据"；
4. **状态机与边界必须闭环**——越界枚举、负值、缺字段、超长、并发重复。

### 发现一：内容留痕导出把控制面打到 OOM（#1023）

列表接口 200、导出接口 **502**——随后在容器日志里看到 `Terminating due to java.lang.OutOfMemoryError`，容器重启（`Up 2 seconds`）。三次复现，秒秒对应。

机制在读代码时看清楚了：`exportCsv` 把最多 `EXPORT_LIMIT = 50 000` 行**一次性读进 `List`**（每行带密文）→ 逐行解密 → 全部拼进 `StringBuilder` → `toString()` 再复制一份 → 才交给响应写出。演示库 **5 482 行 / 107 MB 密文 / 平均 20 KB**：107 MB 字节数组 + ~110 M 字符的 CSV（Java 字符串 UTF-16 → ~220 MB）+ 扩容峰值，448 MB 堆必炸。**"按行数设上限"对内存根本不是约束**——50 000 行在 20 KB/行时是 1 GB。

改成 keyset 分页（500 行/次，`(occurred_at DESC, event_id ASC)` 的严格全序游标）+ 逐行解密写入响应体；`count(*)` 先给截断头（流式响应一旦开始就改不了头）；审计仍在写体前落库。**线上复验**：114 MB 导出成功（94s）、容器 uptime 不重置、无 OOM、GC 只有正常 Young GC。守卫：单测捕获每次查询的 `LIMIT`（旧实现是 1 次 50001 → 红），IT 播 530 行跨分页边界证明不丢行。

顺带一个假警报：`grep OutOfMemoryError` 命中的第一行是 JVM **启动横幅**里的 `-XX:+ExitOnOutOfMemoryError`——这个坑仓里早就记过，这次又踩了一次。

### 发现二：写面校验缺口族（#1019，与 #969/#970/#1005 同族）

探针里 `POST /admin/teams {"name": <201 字符>}` 回 **409 `RESOURCE_CONFLICT`**（文案："重复或引用不允许"），而真实原因是"超过列宽"。读代码后确认这是一族：团队/项目的四个写面**没有任何 Bean Validation**，宽度与必填只存在于**数据库列**和**前端**两处（`NextProjectsView.vue:349` 的注释自己写着 "rules live next to the field instead of only in a 409 body"）；`createUser` 校验了 username 却漏了 `display_name`，而它自己的 update 路径早有 `DISPLAY_NAME_INVALID`。

修法是本仓已有的惯例（`AdminAgentController` 就在用 `@NotBlank @Size`）：DTO 补约束 + 控制器加 `@Valid` → **400 `VALIDATION_FAILED` + `fieldErrors` 点名字段**。边界值照常可用（200 字符团队名、64 字符项目代码），真正的重复 code 仍是 409。

**契约显式化**：CI 的 breaking-change 门禁按设计拦下了它（"property became required"）——这正是该门禁存在的意义。按 #838 的先例，把 OpenAPI 基线与前端类型**从合并后的代码重新生成**（不是手工合并那行单行 JSON），让收紧成为被评审的契约变更而不是意外。

### 发现三：开放管理面的孪生入口不在修复范围内（#1021）

做了一次**全控制器约束扫描**（79 个控制器里 15 个有未加约束的 `*Request` record），再逐条核到服务层，结论：多数不是缺口（服务层拦得住，如 MCP 超时的 `400 MCP_TIMEOUT_INVALID`）——但 **#969/#970 的在办修复（PR #971）只覆盖会话面的两个控制器**，而 `OpenAdminWebhooksController` / `OpenAdminAlertRulesController` 调用的是**同一批服务**，请求体同样零约束。也就是说：#971 合并后，`/api/v1/admin-api/webhooks` 上"secret 为 null → NPE → 500"原样存在——而这条面正是给机器用的。已立 #1021（建议把边界下沉到服务层，否则只补入口的修法会在下一个入口重演）。

### 探针扫清的面（"没有发现"同样要记）

- **跨租户引用**：普通用户读管理员项目成员 / 读他人授权 / 用他人 grant 建 Key / 轮换他人 Key —— 403 或 404，无误收；
- **SSRF 形态**：webhook 的 loopback / 云元数据 / compose 内网名 / `file://` 全部 400 `WEBHOOK_URL_REJECTED`；MCP 端点同理 `MCP_ENDPOINT_INVALID`；
- **生命周期状态机**：ROTATING 不可再轮换/停用、REVOKED 不可启停改名、旧 secret 宽限 PT0S 立即 404 —— 逐条与文档一致；
- **边界**：负值/超大配额、时间区间倒置、未知枚举、10k 字符查询值 —— 全部 400 且分类正确。
...
## 2026-09-20 失败被画成「空」：一次故障注入验收的副产品（#1065）

本轮起点是「跑一遍全站验收」：56 项 API 冒烟 **56/56**、35 个路由的渲染体检全绿（无截断文本、无坏图、无异常字面量、无文档级横向溢出）。既然全绿，就换一个维度——**让接口失败**，看页面说什么。用一台本地故障注入反代（按路径前缀回 5xx，其余透传）实测三页：

- `/app/keys`：`/api/v1/me/virtual-keys` 500 → 页面写「共 0 个」，并渲染「等待管理员开通 / 你的账号还没有被加入任何项目」+ 两步行动指引——**把一次请求失败诊断成了权限问题**；
- `/app/users`：500 → 「共 0 个账号，0 个正常」+「还没有用户」；
- `/app/admin-usage`：四处 500 → 「暂无趋势数据」「没有用量记录」「共 0」。

三页都有 `.ui-alert--error`，但空态与计数照旧输出——**失败的信号在，却被一句更响的「没有数据」覆盖**。与 #960（非数组崩渲染）、#961（缺字段画假值）同族：把「没读到」与「没有」混为一谈。
...
而「上游凭证」列表会把 `?credentialId=` 深链进授权页（`NextCredentialsView.vue:547-555`，来自 #657 / PR #700），
授权页的筛选完全从 URL 推导（`NextGrantsView.vue:174-182`，该文件只 `import { useRoute }`，从不回写 URL），
于是点标签栏或左侧导航
