...
## 15. 诊断陷阱速查（提案 · 待 owner 认可）

> 来源：2026-09-17/18 多会话排障复盘（三例误判 + 三条映射陷阱 + 一组验收陷阱，均经源码/实测逐条核实）。
> 定位：不是症状手册，而是**归因方法**：15.1–15.3 的共同形状是「**症状指向错误的层**」，行动点一律是
> "**去那个层之外找信号**"；15.4 是「**绿灯指向错误的执行上下文**」，行动点是"**先证明红灯会亮**"。
> 15.5–15.8 为 2026-09-18 部署线事故复盘的补充：共同形状是「**证据的强度被高估**」——
> 返回码当就绪、断言比系统更严格、失败语太宽容、源码读错 revision。
> 是否收录/措辞由 owner 审阅本提案时定；未获认可即撤。
...
| 构建静默卡死 | 编译本来就慢 | Maven 依赖下载挂起（到 CDN 的 ESTABLISHED 连接零数据） | **CPU 时间增长**（停滞=卡死，而非看日志）；kill 重试 |
...
与前三条不同族：症状不指向错误的**层**，而指向错误的**执行上下文**——测试报出的绿（或红）
不是被测对象的结论。两例同形（2026-09-18 并入）：
...
变红，再相信绿灯；同族既有先例见 #754（CSV 表头错位近一年未被 contains 式断言抓到）。

### 15.5 「命令返回了」≠「服务就绪了」（部署线上的两次实测，2026-09-18）

同一族，都是"把『进程在』读成『服务可用』"：

| 现象 | 看起来像 | 真实原因 | 行动点 |
|---|---|---|---|
| 部署脚本冒烟在**正常**部署上报 502 | 上游挂了 / 配置错了 | nginx 的上游地址是**启动时解析**的：后端容器刚换了 IP，而 portal 未重启，`/api` 全部 502 直到重启 | 换任何后端容器后必须 `restart`/`reload` portal；冒烟要放在重启**之后** |
...
### 15.6 不可见字符：两个"一模一样"的值不相等

- **现象**：断言报 `ASSERT FAILED … MIQROKEY_REGISTRATION_ENABLED='false' but …/.env says 'false'`——
  两侧打印逐字相同；`cat -A` 当下也看不到差异（文件在报错之后被别的会话改写过，事后才知道那两行当时带 CR）。
- **真实原因**：`.env` 是**混合行尾**（CRLF+LF），比较取的是**文件原始字节**（`false\r`），而容器里是
  compose 解析后的值（`false`）。**这条断言比它所检查的系统更严格**，于是它查的是"文件的行尾是否干净"，
  而不是它存在的理由——"配置有没有送达"。
- **行动点**：
  1. 比较要**按被检查系统的语义归一化**（compose 的 dotenv 容忍 CR、trim 空白、剥一层引号）；
  2. **失败信息必须自证差异**：两侧长度相同就直接 `od -c`/`printf '[%s]'` 打印字节，否则下一个人
     看到的就是"两个一样的值不相等"。
- **同族提醒**：**误报的检查会被无视**（over-strict）与 **跑不起来的检查等于没有检查**（见 15.7）
  是同一枚硬币的两面——两次都让"这条断言到底证明了什么"失效。

### 15.7 宽容的失败语会把检查变成摆设

| 现象 | 看起来像 | 真实原因 | 行动点 |
|---|---|---|---|
| 脚本 `exit 0` 并打印"部署完成"，而站点其实起不来 | 部署成功 | 证书挂载错位 → portal 崩溃重启 → 冒烟拿到 `000`，而 `000` 被分类成"天气"（warning，不判死） | 把"**我们自己的 bug**"与"**外部天气**"分开：URL 由脚本自己派生时，`000` 应当**判失败**；只有用户显式指定目标时，才按"这台机器绕不出去"处理 |
...
| 冒烟"通过"，但冒烟的 URL 被自己的解析 bug 拼坏了 | 检查过了 | 派生 URL 时没做 15.6 的归一化，CR 进了 URL → 够不到 → 判成天气 | 派生输入先归一化；并保证"派生路径的失败不打折" |

共用行动点：**通过语要精确到"这次到底证明了什么"**。在收尾语上多加一个词（`verified`）比在
断言里多加一条检查便宜得多，而前者正是把无配置容器放行的那条路。

### 15.8 读源码要读**已合并的 revision**（工作树会停在旧 commit）

- **现象**：核查"新加的 `quote()` 有没有接线"时，得出"定义了却从未被调用"——**差点发出一条假发现**。
- **真实原因**：读的是**工作树里的文件**，它停在旧 commit；合并版本里 `join()` 早已改为逐格
  `quote(v)`。
- **行动点**：判断"某改动是否生效/接线"一律用 `git show origin/<branch>:<path>`（或 PR 的 merge
  commit）；工作树只用于**编辑**，不作为事实源。与 15.3 同族：**别把没验证的输入当事实**——
  "我看到的源码"也属于输入。
