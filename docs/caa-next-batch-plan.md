...
| # | 缺口 | 现状行为 | 目标 |
|---|---|---|---|
| ① | 建 Key 默认体验 | 需先选主项目、再手动勾附加项目 | **默认全选**用户全部**可绑定**项目（可取消勾选；见 §1.3） |
...
- `frontend/src/views/next/NextKeysView.vue`：
  - 附加项目多选（`extraProjectIds`）默认值 = `projectsForGrant` 中**除主项目外、且持有同一 provider product 授权的全部**（即默认全选*可绑定的*）；用户可取消；
  - 主项目下拉默认 = 可选项目列表第一项（现状可能为空，改为默认选中），消除"先手动选主项目"这一步；
  - 文案：多选标签下移一行说明"选定授权后默认勾选全部**可绑定**的项目——一把 Key 全项目可用；如只需部分项目可取消勾选；未具备同一产品授权的会一并列出但默认不勾选"。
...
- 验收清单新增"新拓扑主路径"（并入主清单执行）：
  1. 普通用户建 Key 表单打开 → 主项目已默认；**选定授权后**其余**可绑定**项目已全选（截图级检查；口径见 §1.3）；
  2. 直接提交（不调整任何项目）→ 创建成功，`boundProjects` 含全部**可绑定**项目；
  3. 两个项目目录（或两个项目标签）各发一次真实推理 → 用量报表按项目分开、每小时表按项目分行；
  4. 取消勾选一个项目再创建 → 该 Key 对该项目绑定缺失（声明该项目 → 403 CONTEXT_NOT_ALLOWED）。

### 1.3 落地后的两处订正（2026-09-20）

本节写的默认值口径在落地后被**收窄过两次**，都是「默认值只在默认情况下成立」这条：

1. **#646 落地时的口径是「`projectsForGrant` 除主项目外的全部」**——它把「有**任意**授权」当成了「可绑定」。服务端要求每个附加项目持有**同一 provider product** 的授权（`VirtualKeyService.java:173-179`，否则 409 `PROJECT_GRANT_MISSING`）。于是**用户不改任何选项直接提交就可能 409**——与 Default-All 的初衷（省事）正好相反。而字段提示当时就写着「附加项目需已具备同一供应商产品的授权」：**默认值与它自己旁边的说明互相矛盾**。
2. **修法最终不是「在某个时机重算一次」，而是把选择改成派生**（`bindableExtraProjectIds` + 用户显式偏离）。原因：只要「选中集合」是被**存储**的，就必然要在某个触发点重算它，而两个方向都错——重算会**复活用户明确取消的勾选**（等于静默绑定他移除掉的项目），不重算则**留下过期的不可绑定项**交给服务端拒绝。两种都从表单可达。

过程记录：#1157 / PR #1165（第一条的实现由独立复审证伪并带回第二条；两条都在同一 PR 内修复）。
...
- **不变量**：策略的凭证与项目 grant 的资源分离（见 §2.6 开放问题 Q1）；归属未知永不借用具体项目的 grant/凭证；单绑定 Key 不受影响——它在后缀步（命中记 `RESOLVED_SUFFIX`）或唯一绑定步（未命中记 `SOLE_BINDING`）就已解析完毕，够不到策略所在的第 4 步。
...
| 单绑定 Key | 不变（在后缀步/唯一绑定步解析完毕，够不到策略）|
