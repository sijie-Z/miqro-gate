...
- 请求前置预检（#553）：鉴权与模型授权通过后、缓存查询与上游调用之前，按 UTF-8 码点统计整个已缓冲 body 的字符数，超过 `MIQROKEY_GATEWAY_CONTEXT_LIMIT_THRESHOLD_CHARS`（默认 `200000`）→ `413 context_limit_exceeded`，不连接上游。该判定只读字节、不解析/不重写 body（转发字节不变），日志与错误体只出现字符数与阈值，不含请求内容；命中计数 `miqrokey_gateway_context_limit_rejected_total`（零标签）。`MIQROKEY_GATEWAY_CONTEXT_LIMIT_ENABLED=false` 可整体关闭。
