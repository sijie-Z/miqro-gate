...
- 重试窗口以**首字节**为界，**不以「请求体是否已送达」为界**：尚未观察到上游首字节、且失败属连接阶段错误（非任何超时）时，可按统一策略进行一次安全重试；一旦出首字节（流式与非流式）即禁止重试。请求体按 chunked 直通转发，上游可能在**读完整个请求体之后**才断开连接，此时上述判定仍会重发：非幂等请求存在被上游重复执行/重复计费、而网关侧只有一条生命周期记录的可能，风险披露见 `docs/architecture.md:159`（#1387）。
