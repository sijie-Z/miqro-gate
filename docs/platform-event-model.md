...
> **本稿只定语义，不含实现。**本轮不写 Java、不写 SQL、不建表、不写迁移、不改任何生产代码；文末 §7 的 schema 内容一律标注「未定稿，等 owner 拍板」。
> **状态：已拍板（2026-09-20，Q1–Q4 全部按推荐 A）**；本稿口径自此生效，schema 与 writer 按 §7.1 路线甲进入实现轮。
>
...
MiQroGate 现有告警链只认识一种事件：「某个租户的事实行，被该租户的规则评估命中，投递到该租户的 webhook」。而队列饱和这类**系统级、无租户语义**的事实放不进这个模型，目前的落地方式是「事实行挂到默认 seed 租户下，让 seed 租户的规则来评估」——它已经跑通，但只是三个特例拼出来的形状，没有成文语义。本稿把这条形状固化成两类事件的模型（§1），定义它们的生命周期（§2）、扇出与去重（§3）、规则所有权与匹配语义（§4）、失败语义与「不递归」原则（§5），并把必须由 owner 拍板的四个口径集中在 §8。**先语义、后 schema**：本轮到此为止，事实表长什么样由下一轮按 §8 的拍板结果决定。
...
- 不做 schema、不做网关侧 writer、不做控制面 dispatcher/评估改动——它们全部是**下一轮**的实现内容。
- 不做扇出（§3）、不做 dead-letter/重放（§2.4）、不做保留清理（§2.5）、不做告警链自监控（§5.3）——前三项等 §8 拍板，末一项按 §5.3 的不递归原则另行设计。
- 不引入新中间件。平台事实的通道就是「网关写事实行、控制面读事实行」的共享 PostgreSQL 模式（V60 已证明可行）；本设计不依赖 Kafka 或任何新组件。
- 不修改 `docs/operations-runbook.md` 等的既有措辞（含 §2.4 记录的 dead-letter 漂移）——文档级修正是下一轮交接的一部分。

**交接句（给下一轮）**：本稿是语义基线；owner 按 §8 拍板后，下一轮按拍板结果实现「schema（事实表/迁移）+ writer（网关侧）+ evaluator 分支（控制面侧）+ 类型注册 8 处 + 测试」，并在实现时同步回填 `docs/api-contract.md`、`docs/database-schema.md`、`docs/operations-runbook.md` 与 `docs/feature-backlog.md:27` 的 F07 行。

---

## 7. 给下一轮的 schema 建议（问题 g —— 仅建议，**未定稿，等 owner 拍板**）
...
### 7.2 路线乙（备选，等 Q2 拍板）：通用 `platform_event_signal` 表
...
## 8. 待拍板（一次一问 —— 每条：选项 / 代价 / 推荐）

> 用法：请 owner 按 Q1→Q4 顺序逐条拍板；每条都给了我方推荐，但口径归 owner。拍板结果回填本节的「拍板记录」空行后，下一轮据此开工。
