...
| 密钥凭证类型 | API Key |
| 生成方式 | 自定义 |
| 凭证内容 | sk-5db73b83d1086f93dff72203312fae（仅作为测试使用） |2. 创建“消费者组”：在目标 AI 网关实例详情页中，菜单栏左侧选择消费者管理 > 消费者组管理 > 新建名为 test-group 的消费者组。3. 创建“消费者”：在目标 AI 网关实例详情页中，菜单栏左侧选择消费者管理 > 消费者管理 > 新建名为 test 的消费者。| 参数 | 填写说明 |
| 消费者名称 | test |
| 所属消费者组 | test-group |
