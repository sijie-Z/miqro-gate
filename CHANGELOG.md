...
- **配额水位的定价口径（#943）**：COST 配额的水位只含**已定价**用量——窗口里发生在没有生效价目时的部分记 0，于是「未定价的窗口」与「确实没花钱」在配额页读成同一个数（`¥0.00` / `NORMAL`），而用量页对同一批事件明确标着「未定价」。现在 **COST 规则**的响应带上与用量 API 同名的 `pricingStatus` / `unpriced`（#766 起的口径），管理端配额页与用户端「我的配额」在数字旁标「未定价」并说明这是**下界**；TOKENS/REQUESTS 规则不带这两个字段（token 与请求数与定价无关）。**执法语义未动**：`EXCEEDED` 仍按同一个数判定，「未定价窗口该不该拒绝 `REJECT` 规则」是 #943 的待决问题②，留待拍板。
...
- **入站 MCP SSE 会话帧缓冲有界化 + 溢出终止（#433）**：会话帧通道原用无参 unicast
  `onBackpressureBuffer`（**无界**——实测：0 需求订阅者下连发 300 帧 `tryEmitNext` 从不失败）：
  慢读者下响应帧无界堆积（`MAX_SESSIONS` 只限会话数、不约束字节数），emit 失败也仅 debug 日志、
  会话不自清——响应静默搁浅、客户端挂死。修复=有界缓冲（256 帧 `MAX_BUFFERED_FRAMES`）+ emit
  失败 WARN 并终止会话（complete + 注册表移除，幂等），客户端见流结束按 MCP SSE 语义重连；
  单测红→绿（0 需求订阅者 + 300 帧 → 会话从注册表移除；原始代码下红灯）。
- **备份工具链三处修复（#438）**：①保留策略此前只做计数上限（最新 11 个），脚本注释与 runbook 承诺的
  「日历周」语义（7 日 + 4 周 ≈ 一个月回滚窗口）未实现、`NEWEST_WEEKLY` 死变量——实际窗口仅 11 天；
  重写为日历周算法（最新 `DAILY_KEEP` 日 + 其余每 ISO 周最新 1 个、至多 `WEEKLY_KEEP` 周），抽为
...
- **修复用量队列冲刷节奏致承压短少（#424）**：全量 verify 下 soak 短少 697/7982 行——根因=每请求 3 事件
  ...
  DROP 丢事件（缺失行数 ≈ 丢事件/3，算术吻合；诊断复跑实证：offer 总数=published+dropped 精确守恒，
  丢弃全在入队端、入库端零丢失——负载下写入端 ~5s 停顿使队列打满即拒）。修复三件套：冲刷周期默认
...
- **修复 CSV 导出的公式注入面（#430）**：前端成本/用量/ROI 三处导出统一收口到共享 `csvCell()`
  （结构引号 + **公式前缀守卫**：`=`/`+`/`-`/`@`/TAB/CR 开头的值加单引号前缀，表格软件按文本渲染）——
  `row.label`（项目/消费者/模型名）为用户可控单元格的真实注入面；ROI 一并获得完整引号（原为裸拼）。
  后端审计导出 `quote()` 同步加防（审计列为 JSON blob 起始/服务端受控，**结构上免疫**，此为纵深防御）；
  审计正常值格式与既有断言不变。测试：BE 单元红→绿 2/2（审计 IT 6/6 保持）+ FE `csv.spec` 3 用例。
