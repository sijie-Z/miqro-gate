...
/**
 * Moonshot/Kimi usage observer（G3.5）。SPI 契约：observer 由适配器在请求前创建， 调用方在解析上游响应后通过
 * {@link #onUsage} 恰好回调一次；观察绝不改写或阻塞 代理字节流。
 *
 * <p>
 * Kimi Code 与 Moonshot 按量的 usage 形状沿用 OpenAI/Anthropic 生态约定，解析 逻辑共享
 * {@link TokenUsageParser}（{@code prompt_tokens} / {@code completion_tokens} /
 * 缓存字段；Anthropic 入口字段同约定）。
 * </p>
 */
public final class MoonshotKimiUsageObserver implements UsageObserver {

    private final UsageContext context;
    private UsageObservation last;

    public MoonshotKimiUsageObserver(UsageContext context) {
