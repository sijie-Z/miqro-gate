...
@DisplayName("AliyunBailianUsageObserver (G3.8)")
class AliyunBailianUsageObserverTest {
    ...
    void parsesOpenAiCompatibleShape() {
        String json = "{\"model\":\"qwen3-coder-plus\",\"usage\":{\"prompt_tokens\":1200,"
                + "\"completion_tokens\":800,\"total_tokens\":2000,\"prompt_cache_hit_tokens\":500,"
                + "\"prompt_cache_miss_tokens\":700}}";

        UsageObservation observation = AliyunBailianUsageObserver
                .parseResponse(MAPPER, json.getBytes(StandardCharsets.UTF_8)).orElseThrow();

        assertThat(observation.modelId()).isEqualTo("qwen3-coder-plus");
    ...
    void parsesAnthropicShape() {
        String json = "{\"message\":{\"usage\":{\"input_tokens\":600,\"output_tokens\":400,"
                + "\"cache_read_input_tokens\":200,\"cache_creation_input_tokens\":100,\"model\":\"qwen3.6-plus\"}}}";

        UsageObservation observation = AliyunBailianUsageObserver
                .parseResponse(MAPPER, json.getBytes(StandardCharsets.UTF_8)).orElseThrow();

        assertThat(observation.modelId()).isEqualTo("qwen3.6-plus");
    ...
    void toleratesEmptyAndMalformedBodies() {
        assertThat(AliyunBailianUsageObserver.parseResponse(MAPPER, "{}".getBytes(StandardCharsets.UTF_8))).isEmpty();
        assertThat(AliyunBailianUsageObserver.parseResponse(MAPPER, "not json".getBytes(StandardCharsets.UTF_8)))
                .isEmpty();
    }
    ...
    void observerBoundToContextAndStoresLast() {
        UsageContext context = new UsageContext(UUID.randomUUID(), UUID.randomUUID(), UUID.randomUUID(),
                ...
                Instant.now());
        AliyunBailianUsageObserver observer = new AliyunBailianUsageObserver(context);

        assertThat(observer.context()).isSameAs(context);
        assertThat(observer.lastObservation()).isEmpty();

        UsageObservation first = new UsageObservation("qwen3.6-plus", 10L, 5L, null, null, null, null, null,
