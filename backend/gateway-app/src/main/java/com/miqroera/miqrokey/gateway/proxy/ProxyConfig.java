...
@EnableConfigurationProperties({ProxyTargetProperties.class, ClientAddressProperties.class,
        ContextLimitProperties.class})
public class ProxyConfig {

    @Bean
    public WebClient proxyWebClient(ProxyTargetProperties properties, UpstreamTargetValidator upstreamTargetValidator,
        ...
        HttpClient httpClient = HttpClient.create(provider)
                // DNS-rebinding TOCTOU defense: every hostname is re-validated
                // at connect time and the connection is pinned to the
                // validated address (blocking work on the credential-decrypt
                // scheduler; SNI and Host keep the original hostname).
                .resolver(new ValidatingAddressResolverGroup(upstreamTargetValidator, credentialDecryptScheduler))
