 ...
 * TOKENS/REQUESTS and the priced CNY cost for COST (#683). {@code enforcement}
 * reports what an exhausted rule does: ALERT only (default) or REJECT, i.e.
 * 429 until the window rolls over (#684).
 */
public record QuotaRuleView(UUID id, QuotaScopeType scopeType, UUID scopeId, String scopeName, String scopeTag,
        QuotaMetric metric, QuotaPeriod period, long limitValue, int warnPercent, QuotaRuleEnforcement enforcement,
        QuotaRuleStatus status, BigDecimal used, BigDecimal usedPct, String level, Instant windowFrom,
        Instant windowTo, Instant createdAt, Instant updatedAt, long version) {
}
