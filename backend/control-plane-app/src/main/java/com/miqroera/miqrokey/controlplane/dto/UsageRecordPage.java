...
public record UsageRecordPage(List<UsageRecordView> items, long page, long size, long total) {
     ...
     * means the row, not the whole number, is 未定价: at least one non-zero token
     * type has no price, so {@code cost} holds only the priced dimensions and is a
     * lower bound rather than a total. It is the amount the report books for this
     * row; the flag is what says not to present it as the full amount.
     * </p>
     */
