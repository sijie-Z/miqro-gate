 ...
 * 规则写进 {@code quota_enforcement}；网关侧 {@code JdbcRouteSnapshotLoader} 读该表
 * 生成 {@code quotaBlockedUsers}，{@code QuotaGate} 据此返回 429 quota_exceeded。 于是
 * {@code UsageDeletionService.confirm()} 物理删掉当前窗口内的
 * {@code usage_event} 行之后，下一次评估算出的 watermark 归零， 规则不再 EXCEEDED →
 * 该 scope 从 {@code quota_enforcement} 消失 → 网关停止 429。
 * </p>
 *
 * <p>
 * 文档与领域模型对这条 429 的承诺是「直到窗口结束、或者管理员提高限额才解除」
 * （{@code QuotaAction.REJECT} / {@code QuotaRule} javadoc、{@code QuotaGate}），
 * 用法删除完全不在解除条件里——管理员以为自己在做数据保留清理，实际后果是把 超限流量重新放行。
 * 本测试断言该承诺成立：删除之后 block 必须还在。
 * </p>
 *
 * <p>
 * 规则用 PROJECT scope：USER scope 的聚合要 JOIN virtual_keys 才能按 user 过滤，PROJECT
 * scope 的谓词直接落在 {@code usage_event.project_id} 上。两者走的是同一条
 * watermark → verdict → {@code quota_enforcement} → 网关快照 的链路，本测试只关心该链路
 * 的输入（窗口内用量）被删除后的结果。
 * </p>
 */
...
class QuotaBlockSurvivesUsageDeletionIntegrationTest {
    ...
    /** The project the rule blocks and the project the seeded usage is attributed to. */
    ...
    void usageDeletionMustNotLiftAnExceededQuotaBlock() {
        ...
        System.out.println("[PH67] window usage_event rows=" + windowEventCount() + " window tokens="
                + windowTokens() + " (limit " + LIMIT_TOKENS + ") block rows after 1st evaluate=" + blockedBefore);
        ...
        assertThat(blockedAfter).as("管理员删除用量后，被 REJECT 规则拦住的 scope 不得被自动解封："
                + "quota_enforcement 里必须仍然有这条规则（网关的 429 只由该表驱动）").isEqualTo(1);
    }
    ...
    void staleVerdictIsDroppedOnceItsWindowHasEnded() {
        ...
        jdbc.update("DELETE FROM usage_event WHERE tenant_id = :tenantId", new MapSqlParameterSource("tenantId", TENANT));
        ...
        assertThat(blockedRuleRows())
                .as("window_end 已过 + 当前窗口未超限 → 判定必须消失，否则被拦的 scope 永远解不开").isZero();
    }
    ...
    private void seedRecordedBlock(Instant windowEnd, Instant blockedAt) {
        jdbc.update("""
                INSERT INTO quota_enforcement (rule_id, tenant_id, scope_type, scope_id, metric, period, window_end,
                    blocked_at)
                VALUES (:rule, :tenantId, 'PROJECT', :project, 'TOKENS', 'DAILY', :windowEnd, :blockedAt)
                """, new MapSqlParameterSource("rule", RULE).addValue("tenantId", TENANT)
                .addValue("project", PROJECT).addValue("windowEnd", Timestamp.from(windowEnd))
                .addValue("blockedAt", Timestamp.from(blockedAt)));
    }
    ...
    /** The token reading {@code QuotaWatermarks} derives for the blocked project today (UTC). */
    ...
    private List<String> blockRows() {
        return jdbc.queryForList("SELECT scope_type || ' ' || scope_id || ' until ' || window_end FROM quota_enforcement"
    ...
    /**
     * The project must be a real {@code projects} row: the watermark aggregation groups by
     * {@code "project"}, and that group spec INNER JOINs {@code projects} on
     * {@code p.id = ue.project_id}. A dangling project id would drop every usage row.
     */
    ...
    private void seedUsage() {
        jdbc.update("""
                ...
                """, new MapSqlParameterSource("tenantId", TENANT).addValue("projectId", PROJECT)
                .addValue("count", WINDOW_EVENTS));
    }
