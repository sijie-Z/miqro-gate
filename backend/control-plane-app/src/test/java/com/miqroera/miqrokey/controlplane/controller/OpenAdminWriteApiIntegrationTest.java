...
class OpenAdminWriteApiIntegrationTest {
    ...
    void missingRuleTypeIsRejectedOnTheMachineFace() throws Exception {
        // Deliberately not routed through assertBothFacesReject: that helper asserts the
        // documented DTO answer (400 VALIDATION_FAILED + fieldErrors), while `type` is
        // the one field the DTOs leave to AlertRuleService and the service answers with
        // its own code. The machine face has no DTO constraints of its own either, so it
        // reaches the same service check and must land on the same 400.
    ...
    void bothFacesRejectTheSamePayloadsIdentically() throws Exception {
        ...
        MvcResult created = mockMvc
                .perform(post("/api/v1/admin-api/webhooks").header("Authorization", "Bearer " + machineSecret)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(Map.of("name", "guard-probe", "url",
                                "https://example.com/h", "secret", "whsec-x", "timeoutMs", 5000))))
                .andExpect(status().isCreated()).andReturn();
