 ...
 * SHARE OF x}（外加同样形状的级联删除）。该查询只有 {@code usage_event_id} 一个 条件，而 V63 建的索引是
...
class UsageDeletionCascadeIndexIntegrationTest {
    ...
    void foreignKeyCheckQueryUsesIndex() {
        ...
        assertThat(plan)
                .as("PostgreSQL 为外键 usage_adjustments_usage_event_id_fkey 执行的正是这条查询，" + "它必须能走 usage_event_id 前导的索引")
