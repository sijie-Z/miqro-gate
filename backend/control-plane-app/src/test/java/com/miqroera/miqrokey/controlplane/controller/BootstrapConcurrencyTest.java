...
@Tag("integration")
@DisplayName("Bootstrap concurrency tests")
class BootstrapConcurrencyTest {
    ...
    @DisplayName("concurrent bootstrap with two distinct usernames: exactly one admin committed")
    void concurrentBootstrapExactlyOneSucceeds() throws Exception {
        int threads = 2;
        ...
        for (int i = 0; i < threads; i++) {
            final String username = "concurrent_admin_" + i + "_" + UUID.randomUUID().toString().substring(0, 8);
            tasks.add(() -> {
                latch.await();
                MvcResult r = mockMvc
                        .perform(
                                post("/api/v1/auth/bootstrap").contentType(MediaType.APPLICATION_JSON)
                                        .content(objectMapper.writeValueAsString(new BootstrapRequest(
                                                AuthIntegrationTest.BootstrapHelper.secret(), username, "Test"))))
                        .andReturn();
        ...
        for (Future<Integer> f : futures) {
            int status = f.get(1, TimeUnit.SECONDS);
            if (status == 201)
                successCount++;
            else
                failureCount++;
        ...
        assertThat(successCount).as("exactly one bootstrap must succeed").isEqualTo(1);
        assertThat(failureCount).as("the other bootstrap must be rejected").isEqualTo(1);
