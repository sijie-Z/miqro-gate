...
ALTER TABLE key_project_binding
    ADD CONSTRAINT fk_key_project_binding_grant
        FOREIGN KEY (grant_id) REFERENCES project_provider_grants (id) ON DELETE RESTRICT;
...
-- ---------------------------------------------------------------------------
-- 2. 项目标签回填：uuid 前 12 位（去连字符）。50 账号规模下唯一性充分；
