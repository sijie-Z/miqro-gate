-- ============================================================================
-- V63: 用量调整台账（issue #709 / 台账 F20）——追加型修正，绝不覆盖原始事实。
--
-- 编号说明：本迁移最初占用 V62，但 #729 的 usage_event 复合索引（V62，
-- PR #744）先合入了 develop。两支各持一个 V62 会在合并后于启动时报
-- "Found more than one migration with version 62"，故本迁移让号改为 V63。
-- 教训同 V60 那次：**定号前要同时看 develop 树和 open issue 里已登记的号**，
-- 只看当前树会漏掉别处在途的占用。
--
