 ...
 * writer scheduler — never from the gateway hot path, which is the same red
 * line {@link UsageEventWriter} obeys.
 *
 * <p>
...
public interface QueueSignalWriter {
    ...
    /**
     * Writes one saturation fact. Implementations must be idempotent per signal:
     * the bus restores an unwritten delta and retries it on the next cycle, so a
     * failed call must not have left a row behind.
     */
