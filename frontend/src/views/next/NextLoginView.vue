...
.gate-hero {
  ...
  min-width: 0;
  min-height: 100vh;
  /* Width-relative text sizing (cqw) keeps the copy inside the artwork's
     blank left column at every viewport — no drift against the bitmap. */
...
.hero-copy {
  ...
  align-self: start;
  /* The artwork's blank column is ~34% of its width — never exceed it. */
  max-width: min(560px, 34cqw);
  padding-top: 56px;
}
.hero-eyebrow {
  margin: 0;
...
.hero-copy h1 {
  margin: 24px 0 24px;
  /* 4.7cqw keeps 7-char Chinese lines inside the blank column at any width;
     56px is the artwork's baked headline size (upper bound). */
  font-size: min(56px, 4.7cqw);
  line-height: 0.99;
...
.hero-footer {
  ...
  flex-wrap: wrap;
  gap: 6px 20px;
  /* Footer keeps to the artwork's blank left column as well. */
  max-width: 36cqw;
  margin-left: 12px;
  color: #75829d;
  font-size: 9px;
